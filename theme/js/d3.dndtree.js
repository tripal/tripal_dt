/**
 * @file
 * Contains the D3 JS code to render the demo tree.
 *
 * Requires D3 JS v3.4.11.
 *
 * Most of this code comes from http://bl.ocks.org/robschmuecker/7880033
 * and has been adapted to be used in this module.
 * You could use any other D3JS example (for D3JS 3.4.11) and adapt it the
 * same way.
 *
...
/******************************************************************************/
  ...
  Drupal.tripal_dt.renderD3JSTree = function(treeData) {
    ...
    var root;
    // Label used for nodes without labels. Should not be empty because the
    // label string lenght is used to layout the tree.
    var defaultNodeLabel = '        ';

    // Size of the diagram.
    var viewerWidth = jQuery('#content').width(); // Or jQuery(document).width();
    var viewerHeight = 512; // Or jQuery(document).height();
    ...
    var baseSvg = d3.select("#tripal_dt_tree").append("svg")
    ...
    function update(source) {
      ...
      var nodeEnter = node.enter().append("g")
          // Uncomment to enable node drag-n-drop.
          // .call(dragListener)
          .attr("class", "node")
      ...
      node.select('text')
          ...
          .classed({
            "tripal-dt-tt-node": function(d) { return (d.children || d._children); },
            "tripal-dt-tt-leaf": function(d) { return (!d.children && !d._children); },
          })
          .html(function(d) {
            return d.name;
          })
